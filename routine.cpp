...
//
// Purpose:
//   Sends a CAN FD message corresponding to a click event (single, double, long, etc.)
//   to a linked board, if a valid link exists in the database.
//
// Parameters:
//   Index  : Index (0..15) into the current board's link table (`DB[LABEL].lnk[Index]`)
//   value  : Enum-like constant representing the click type (CLICK_S, CLICK_SS, etc.)
//
// Operation:
//   1. Map the ClickValue to a single-byte parameter code.
//   2. Retrieve the LINK structure from DB[LABEL].lnk[Index].
//   3. Check if the link is valid (not marked as FREE).
//   4. If valid, construct a CAN FD frame and send it.
//   5. If invalid, optionally print a debug message.
//
// Notes:
//   - The link's `.base` field is the DB index of the linked board.
//   - The link's `.sub` field is an offset to be added to the linked board's base CAN ID.
//   - FREE link sentinel values: base = 0xFF, sub = 0x0F.
//   - Uses sendCANFDFrame() helper to transmit the CAN FD message.
//======================================================================================

void Send_Click(uint8_t Index, ClickValue value)
  {
    uint8_t param = 0;                                                                            // Store the encoded click type to be sent in the CAN frame

// Map the ClickValue enum to its corresponding byte value.
// If the value doesn't match any known click type, exit the function early

    switch (value)
      {
        case CLICK_S:   param = CLICK_S;   break;                                                 // Single click
        case CLICK_SS:  param = CLICK_SS;  break;                                                 // Double click
        case CLICK_L:   param = CLICK_L;   break;                                                 // Long click
        case CLICK_SL:  param = CLICK_SL;  break;                                                 // Short-long click combo
        case CLICK_VL:  param = CLICK_VL;  break;                                                 // Very long click
        default:
        return;                                                                                   // Unknown click type → nothing to send
      }

// Retrieve the link entry from the database for the given Index.
// LABEL is assumed to be the current board's DB index.

    LINK link = DB[LABEL].lnk[Index];
    if(link.base != 0xFF && link.sub != 0x0F)                                                     // Check if the link entry is valide
      {   
// Link valide, construct the CAN FD message frame
        CANFDMessage frame;                                                                       // Local variable to hold the CAN FD frame fields
        frame.id   = DB[link.base].CAN + link.sub;                                                // Full CAN ID = linked board's base ID + sub offset
        frame.ext  = false;                                                                       // Standard (11-bit) CAN ID format
        frame.len  = 1;                                                                           // Only one byte of payload
        frame.type = CANFDMessage::CANFD_WITH_BIT_RATE_SWITCH;                                    // Enable CAN FD + BRS (faster data phase)
        frame.idx  = 0;                                                                           // Transmit using TX FIFO slot 0
        frame.data[0] = param;                                                                    // Put the click code into the payload
        sendCANFDFrame(frame.data, frame.len, frame.id);                                          // Send the frame.
      }
    else
      {
        if (IDE) Serial.println(F("INVALID LINK"));                                               // Link is invalid → output a debug message
      }
  }
